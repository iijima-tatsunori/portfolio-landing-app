class AccountsController < ApplicationController
  ...
  before_action :logged_in_user, only: [:new, :create, :show, :index, :edit, :update, :destroy,
                                        ...
                                        :all_general_ledger, :master_general_ledger,
  ...
  before_action :transfer_slip_account_title, only: [:transfer_slip_new, :transfer_slip_create, :transfer_slip_edit, :transfer_slip_update, :transfer_slip_destroy, :all_general_ledger]
  ...
  def transfer_slip_update
    if @account.update_attributes(transfer_slip_account_params)
      ...
      redirect_to journal_books_accounts_url
    else
      flash[:danger] = "入力エラー。"
      redirect_to journal_books_accounts_url
    end
  ...
  def master_general_ledger
    ...
    @general_ledger_accounts = Account.joins(:compound_journals).where(compound_journals: {account_title: @param_account_title}).or(Account.joins(:compound_journals).where(compound_journals: {right_account_title: @param_account_title})).merge(Account.order("accounts.accounting_date ASC"))
    @compound_journals = CompoundJournal.where(account_title: @param_account_title)
  end
